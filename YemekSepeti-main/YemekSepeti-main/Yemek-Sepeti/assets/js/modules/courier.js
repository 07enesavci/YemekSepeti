...
function loadProfileData() {
    ...
    if (!courierNameInput.value) {
        courierNameInput.value = '';
    }

    courierPhoneInput.value = formatPhoneNumber(COURIER_DATA.phone);
    if (!courierPhoneInput.value) {
        // formatPhoneNumber'dan gelen boş değer için
        courierPhoneInput.value = ''; 
    }

    courierVehicleSelect.value = COURIER_DATA.vehicle;
    if (!courierVehicleSelect.value) {
        courierVehicleSelect.value = '';
    }
    // ------
...
function handleProfileUpdate(event) {
    ...
    // Ternary Operatör Dönüşümleri (name)
    if (courierNameInput.value) {
        COURIER_DATA.name = courierNameInput.value;
    }
    // Ternary Operatör Dönüşümleri (phone)
    const rawPhone = courierPhoneInput.value.replace(/\D/g, '');
    if (rawPhone) {
        COURIER_DATA.phone = rawPhone;
    }
    // Ternary Operatör Dönüşümleri (vehicle)
    if (courierVehicleSelect.value) {
        COURIER_DATA.vehicle = courierVehicleSelect.value;
    }
