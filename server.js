...
// connect-mysql2 için yapılandırma (AIVEN CLOUD AYARLARI)
    ...
    console.log('📋 Route dosyaları yükleniyor...');
    routeFiles.forEach(route => {
        try {
            const routePath = path.resolve(__dirname, `routes${route}.js`);
            console.log(`🔍 Route kontrol ediliyor: ${route} -> ${routePath}`);
            
            if (fs.existsSync(routePath)) {
                console.log(`✅ Route dosyası bulundu: ${routePath}`);
                
                // Route modülünü require et
                const routeModule = require(routePath);
                
                // Express router bir middleware'dir (function veya object olabilir)
                if (routeModule) {
                    app.use(route, routeModule);
                    console.log(`✅✅ Route başarıyla yüklendi: ${route}`);
                    writeLog('INFO', `Route yüklendi: ${route}`, { path: routePath });
                } else {
                    console.error(`❌ Route modülü geçersiz: ${route} - modül tipi: ${typeof routeModule}`);
                    writeLog('ERROR', `Route modülü geçersiz: ${route}`, { path: routePath, moduleType: typeof routeModule });
                }
            } else {
                console.error(`❌ Route dosyası bulunamadı: ${routePath}`);
                writeLog('ERROR', `Route dosyası bulunamadı: ${route}`, { path: routePath });
            }
        } catch (error) {
            console.error(`❌❌ Route yüklenirken KRİTİK HATA: ${route}`);
    ...
    console.log('📋 Route yükleme işlemi tamamlandı.');
