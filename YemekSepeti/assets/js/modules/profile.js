...
async function useCurrentLocationForProfileAddress(buttonEl) {
    ...
    navigator.geolocation.getCurrentPosition(async (position) => {
        try {
            ...
            showProfileLocationPreview(lat, lng);

            const resolved = await reverseGeocode(lat, lng);
            if (resolved) {
                const districtInput = form.querySelector('#address-district');
                const cityInput = form.querySelector('#address-city');
                const detailInput = form.querySelector('#address-detail');

                if (districtInput && resolved.district) districtInput.value = resolved.district;
                if (cityInput && resolved.city) cityInput.value = resolved.city;
                if (detailInput && resolved.detail) detailInput.value = resolved.detail;

                alert('Konumdan adres bilgisi dolduruldu. Gerekirse düzenleyip kaydedin.');
            } else {
                alert('Konum alındı ancak adres çözümlenemedi. Alanları elle doldurabilirsiniz.');
            }
        } catch (err) {
        ...
        if (error && error.code === 1) {
            alert('Konum izni reddedildi. Lütfen tarayıcıdan konum izni verin.');
        } else {
...
function showProfileLocationPreview(lat, lng) {
    const previewContainer = document.getElementById('profile-location-preview');
    const previewMap = document.getElementById('profile-location-preview-map');
    const previewCoords = document.getElementById('profile-location-preview-coords');
...
function clearProfileLocationPreview() {
    const previewContainer = document.getElementById('profile-location-preview');
    const previewMap = document.getElementById('profile-location-preview-map');
    const previewCoords = document.getElementById('profile-location-preview-coords');
            ...
            <div class="card-content">
                ...
                <label for="address-detail" class="form-label">Adres Detayı (Mahalle, Sokak, No...)</label>
                    ...
                    <div class="form-group" style="margin-top: -0.25rem;">
                        <button type="button" class="btn btn-secondary" id="use-current-location-profile-btn">📍 Konumumu Kullan</button>
                    </div>
                    <div id="profile-location-preview" style="display: none; margin-bottom: 1rem;">
                        <div style="font-size: 0.9rem; color: var(--secondary-color); margin-bottom: 0.5rem;" id="profile-location-preview-coords"></div>
                        <iframe
                            id="profile-location-preview-map"
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function renderAddresses() {
        ...
        const useLocationBtn = document.getElementById('use-current-location-profile-btn');
        if (useLocationBtn) {
            useLocationBtn.onclick = function() {
                useCurrentLocationForProfileAddress(this);
            };
        }
    ...
    async function handleNewAddressSubmit(e) {
        ...
        const latitude = form.dataset.latitude || null;
        const longitude = form.dataset.longitude || null;
        ...
        const newAddressData = { title, district, city, detail, latitude, longitude, isDefault };
