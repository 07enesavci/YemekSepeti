...
router.post("/addresses", requireRole('buyer'), async (req, res) => {
    try {
        ...
        const latValue = latitude !== undefined && latitude !== null && latitude !== '' ? parseFloat(latitude) : null;
        const lngValue = longitude !== undefined && longitude !== null && longitude !== '' ? parseFloat(longitude) : null;

        if (latValue !== null && Number.isNaN(latValue)) {
            return res.status(400).json({ success: false, message: "Geçersiz enlem bilgisi." });
        }

        if (lngValue !== null && Number.isNaN(lngValue)) {
            return res.status(400).json({ success: false, message: "Geçersiz boylam bilgisi." });
        }

        const now = new Date();
        const result = await db.execute(
            `INSERT INTO addresses (user_id, title, district, city, full_address, latitude, longitude, is_default, created_at, updated_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [userId, title.trim(), district.trim(), city.trim(), fullAddress, latValue, lngValue, defaultFlag, now, now]
        );
        res.json({
            success: true,
