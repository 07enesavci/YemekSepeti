...
router.get("/me", (req, res) => {
    ...
    console.log('🔍 GET /api/auth/me - İstek alındı', {
        hasSession: !!req.session,
        sessionID: req.sessionID,
        isAuthenticated: req.session?.isAuthenticated,
        hasUser: !!req.session?.user,
        cookies: req.cookies ? Object.keys(req.cookies) : [],
        cookieHeader: req.headers.cookie ? 'var' : 'yok'
    });
    
    // Önce session kontrolü yap
    if (req.session && req.session.isAuthenticated && req.session.user) {
        console.log('✅ Session bulundu, kullanıcı:', req.session.user.email);
        return res.json({
            success: true,
            user: req.session.user
        });
    ...
    if (token) {
        try {
            ...
            console.log('✅ JWT token\'dan session oluşturuldu, kullanıcı:', user.email);
            return res.json({
                success: true,
                user: user
            });
        } catch (err) {
            console.log('⚠️ JWT token geçersiz:', err.message);
        }
    }
    
    console.log('⚠️ Session ve token bulunamadı - 401 döndürülüyor');
    res.status(401).json({
        success: false,
