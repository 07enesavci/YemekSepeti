...
router.put("/profile", requireRole('courier'), async (req, res) => {
    try {
        ...
        if (!fullname || !phone) {
            return res.status(400).json({
                success: false,
                message: "Ad soyad ve telefon numarası gereklidir."
            });
        }
        ...
        let updateFields = ["fullname = ?", "phone = ?"];
        let updateValues = [fullname, phone];
        ...
        try {
            ...
            if (dbError.code === 'ER_BAD_FIELD_ERROR') {
                ...
                try {
                    // ALTER TABLE komutlarını çalıştır
                    await db.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS courier_status ENUM('online', 'offline') DEFAULT 'online'");
                    await db.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS vehicle_type ENUM('motorcycle', 'car', 'bicycle') DEFAULT 'motorcycle'");
